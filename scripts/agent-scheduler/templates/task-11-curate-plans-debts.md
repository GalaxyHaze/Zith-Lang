...
```bash
rg -n "<<<<<<<|>>>>>>>|=======" {REPO}/docs/roadmap.md
rg -n "frontend-context.cpp.*pending|compilation-session.cpp.*pending" {REPO}/docs/implementation-debt.md {REPO}/docs/plans/monolith-splits.md
rg -n "Status: Working|Status: Parse skipped|Spec only" {REPO}/docs/plans/0.7.0 {REPO}/docs/roadmap.md | head -80
```
