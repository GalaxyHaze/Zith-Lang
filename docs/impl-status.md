...
| Import resolution | **Working** | `import path`, `import path as name`, `from path`, `export path`, and selectors; platform variants are resolved as `foo.<arch>.<os>.zith`, `foo.<arch>.zith`, `foo.<os>.zith`, then generic `foo.zith`; qualified access works in expressions, types, constructors and methods. `export` injects the target's public symbols and creates one qualified namespace alias per exported prefix so shared-prefix facades fan out to every submodule |
...
| Stdlib allocation | **Working** | `std/memory` ships the strict DAG with `in-place`, `allocator`, `heap`, and `new`; imported `InPlace` conformance and qualified trait calls are deterministically covered by `tests/test-interface-satisfaction.cpp` and `tests/test-generic-hashmap.cpp`. `std/alloc` remains as a legacy compatibility module. The generic `new`/`delete`/`make`/`release` helpers remain proposed until return-only inference and opaque pack dispatch are supported |
...
| Call-result statements | **Working** | Non-void call/dock results used as expression statements must be consumed or explicitly discarded with `_ = expr;` (`E2026`). Void calls and value-producing uses (binding, assignment, return, argument, condition, binary operand, field/index operand, method dispatch) remain legal |
...
| 1001-1009 | Import/parse | `E1001` ExpectedExpr, `E1002` ExpectedSemicolon, `E1003` UnclosedParen, `E1004` ExpectedIdent, `E1005` InvalidImportDepth, `E1006` ImportError, `E1007` TopLevelLetNotAllowed, `W1008` DeprecatedSyntax (`while` -> `for (cond)`; `else if` -> `else (cond)`; `~>` in `when` arms), `E1009` CircularImport |
...
| `?*T` pointer narrowing is local-only | Non-null proof currently follows a nullable-pointer local through `is null`/`not (is null)` branches and early-return checks; derived field/index/arrow expressions and cross-function proofs are not yet tracked |
