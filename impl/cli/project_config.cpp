...
constexpr const char *kKnownKeys[] = {
    "name", "version", "description", "authors", "license", "homepage", "entry",
    "output", "mode", "target_triple", "edition", "src_dir", "bin_dir", "lib_dir",
    "docs_dir", "test_dir", "cache_dir", "include_dirs", "lib_paths", "link_libs",
    "link_flags", "features", "dependencies", "emit_ir", "emit_asm", "strip_debug",
    "lto", "opt_level", "debug_level"};
...
void set_string_field(const toml::table &tbl, const char *key, std::string &field,
    ...
    if (auto node = tbl.get(key)) {
        if (auto value = node->value<std::string>()) field = *value;
        else errors.emplace_back(std::string("Field '") + key + "' must be a string");
    }
}

} // namespace

void ZithProject::build_import_roots(const std::vector<std::string> &extra_dirs,
...
bool try_load_project_from_path(ZithProject &proj, const std::string &project_file,
    ...
    proj = ZithProject{};
    auto parse_result = toml::parse_file(project_file);
    if (!parse_result) {
        local_errors.emplace_back("Failed parsing TOML: " + std::string(parse_result.error().description()));
    } else {
        auto &tbl = *parse_result;
        for (const auto &[k, _] : tbl) {
            if (!is_known_key(std::string(k.str()))) {
                local_warnings.emplace_back("Unknown key in ZithProject.toml: '" + std::string(k.str()) + "'");
            }
        }

        for (const auto *req : kRequiredKeys) {
            if (!tbl.contains(req)) local_errors.emplace_back(std::string("Missing required field '") + req + "'");
        }
        ...
        if (auto opt = tbl["opt_level"].value<int64_t>()) {
            if (*opt < 0 || *opt > 3) local_errors.emplace_back("Field 'opt_level' must be in range 0..3");
            else proj.opt_level = static_cast<int>(*opt);
        ...
        if (auto dbg = tbl["debug_level"].value<int64_t>()) {
            if (*dbg < 0 || *dbg > 3) local_errors.emplace_back("Field 'debug_level' must be in range 0..3");
            else proj.debug_level = static_cast<int>(*dbg);
        } else if (tbl.contains("debug_level")) {
            local_errors.emplace_back("Field 'debug_level' must be an integer");
        }

        const std::filesystem::path base_dir = std::filesystem::absolute(std::filesystem::path(project_file)).parent_path();
        proj.entry = (base_dir / proj.entry).lexically_normal().string();
    }

    if (warnings) *warnings = std::move(local_warnings);
