    ...
    switch (expr->type) {
        case ZITH_NODE_LITERAL: {
            ...
            if (lit->kind == ZITH_LIT_INT || lit->kind == ZITH_LIT_UINT) {
                v.kind = RtValKind::Int; v.i = lit->value.i64;
            } else if (lit->kind == ZITH_LIT_FLOAT) {
                v.kind = RtValKind::Float; v.f = lit->value.f64;
                ...
                v.kind = RtValKind::Bool; v.b = lit->value.boolean;
            }
            return v;
        }
    ...
    switch (stmt->type) {
        ...
        case ZITH_NODE_FOR: {
            ...
            while (true) {
                ...
                if (!payload || !payload->condition) break; // Infinite loop guard if condition is null (though unlikely in for)
                
                // Note: 'step' logic is missing in AST access here, assuming standard C-style for loop structure or simplified
                // If payload->step exists, it should be evaluated here.
            }
            return {};
        }
        case ZITH_NODE_CALL:
