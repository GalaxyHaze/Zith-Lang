...
void CodeGen::emit(const hir::HirModule &hirModule, std::string_view moduleName) {
    ...
    for (size_t i = 0; i < hirModule.getFnCount(); i++) {
        ...
        // Check if this is an extern fn (no blocks = extern)
        bool is_extern = fn.blocks.empty();

        // Also check the AST declaration for the is_extern flag
        if (fn.decl_id != ast::kInvalidDecl) {
            // We can't access the AST builder here, but the HIR already
            // captured the is_extern info via blocks.empty()
        }
...
void CodeGen::emitExternFn(const hir::HirFunction &fn) {
    ...
    // Build function type from HIR params + return type
...
void CodeGen::emitFunction(const hir::HirFunction &fn, const hir::HirModule &mod) {
    ...
    // If the function returns void and no terminator, add ret void
