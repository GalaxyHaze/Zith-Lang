...
ScanResult scan(Parser &parser, import::SymbolTable &syms, bool sema) {
    ...
    while (!tok.is_empty()) {
        ...
        if (tok.peek().is(TokenKind::Struct)) {
            ...
            if (!tok.peek().is(TokenKind::Identifier)) {
                diag.report(Severity::Error, ExpectedIdent, "expected struct name",
                            tok.peek().span);
                tok.advance();
                current_vis       = import::SymbolVisibility::Private;
                current_mod_depth = 0;
            ...
            auto decl = bld.structDecl(
                name, memory::DynArray<std::pair<std::string_view, uint32_t>>(bld.arena()));
            program.decls.push(decl);
            if (!reportIfDuplicate(syms, diag, name, name_span))
                syms.declare(name, current_vis, current_mod_depth, import::SymKind::Struct, decl,
                             name_span);
