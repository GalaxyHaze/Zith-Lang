...
bool Unifier::unify(TypeId a, TypeId b) {
    ...
    switch (kind_a) {
    case TypeKind::Int:
        return std::get<TypeInt>(data_a).width == std::get<TypeInt>(data_b).width;
    case TypeKind::Float:
        return std::get<TypeFloat>(data_a).width == std::get<TypeFloat>(data_b).width;
    case TypeKind::Ptr:
        return unify(std::get<TypePtr>(data_a).pointee, std::get<TypePtr>(data_b).pointee);
    case TypeKind::Array: {
        auto &arr_a = std::get<TypeArray>(data_a);
        auto &arr_b = std::get<TypeArray>(data_b);
        if (arr_a.count != arr_b.count)
            return false;
        return unify(arr_a.elem, arr_b.elem);
    }
    case TypeKind::Struct:
        return std::get<TypeStruct>(data_a).def_id == std::get<TypeStruct>(data_b).def_id;
    case TypeKind::Fn: {
        auto &fn_a = std::get<TypeFn>(data_a);
        auto &fn_b = std::get<TypeFn>(data_b);
        if (fn_a.params.size() != fn_b.params.size())
            return false;
        if (!unify(fn_a.ret, fn_b.ret))
            return false;
        for (size_t i = 0; i < fn_a.params.size(); i++) {
            if (!unify(fn_a.params[i], fn_b.params[i]))
                return false;
        }
        return true;
    }
